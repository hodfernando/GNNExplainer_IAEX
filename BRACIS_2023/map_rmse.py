...
def map_rmse(results_path, rmse, datasetLoader, avg_percent_error, threshold):
    mydir = os.getcwd()
    ...
    df = pd.DataFrame({'geocode': geocode, 'City name': nomemun, 'RMSE': rmse.flatten()})
    ...
    fig = px.choropleth_mapbox(
        ...
        color="RMSE",  # defining the color of the scale through the database
        hover_name="City name",  # the information in the box
        hover_data=["City name", "geocode", "RMSE"],
        title="RMSE COVID-19 cases in Brazil",  # title of the map
        mapbox_style="carto-positron",
        zoom=3, center={"lat": -15.0000, "lon": -55.0000},
        opacity=0.5,
        labels={'RMSE': 'RMSE COVID-19 cases'}
    )
    fig.update_geos(fitbounds="locations", visible=False)
    fig.update_layout(margin={"r": 0, "t": 0, "l": 0, "b": 0})
    # fig.show()
    fig.write_html(
        mydir + f'/{relative_path}/maps/' + file[:-8] + '_' + 'choropleth_mapbox_rmse.html')
